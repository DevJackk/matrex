...
print('\n\27[1;31m￤ Conect is Failed !\n￤ خلي صاحب البوت , يبعت ابدأ في البوت وبعدين دخل الايدي تاني')
create_config(Token)
end
GetUser.result.username = GetUser.result.username or GetUser.result.first_name
print('\n\27[1;36m￤ تم كتابه ايدي المطور , سوف يتم تشغيل البوت علي سورس ماتريكس الان .\n￤Success Save USERID : \27[0;32m['..SUDO_USER..']\n\27[0;39;49m')
matrex = Token:match("(%d+)")
redis:set(matrex..":VERSION",1)
redis:set(matrex..":SUDO_ID:",SUDO_USER)
redis:set(matrex..":DataCenter:",'German')
redis:set(matrex..":UserNameBot:",BOT_User)
redis:set(matrex..":NameBot:",BOT_NAME)
redis:hset(matrex..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3matrex_INSTALL","Yes")
info = {}
...
matrex = Token:match("(%d+)")
our_id = tonumber(matrex)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(matrex..":UserNameBot:")
SUDO_ID = tonumber(redis:get(matrex..":SUDO_ID:"))
SUDO_USER = redis:hgetall(matrex..'username:'..SUDO_ID).username
version = redis:get(matrex..":VERSION")
DataCenter = redis:get(matrex..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..matrex..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(matrex..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(matrex..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(matrex..'group:add'..msg.chat_id_) then 
msg.GroupActive = true
else
msg.GroupActive = false
end

if msg.sender_user_id_ == 1424388256 then 
msg.TheRank =  'البابا جاك'
msg.Rank = 1

elseif msg.sender_user_id_ == 1019610884 then 
msg.TheRank =  'مالك السورس'
msg.Rank = 1
elseif msg.sender_user_id_ == 1038368606 then 
msg.TheRank =  ' مالك السورس '
msg.Rank = 1
elseif msg.sender_user_id_ == 1317966118 then 
...
msg.Rank = 1
elseif redis:sismember(matrex..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(matrex..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(matrex..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(matrex..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(matrex..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(matrex..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(matrex..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(matrex..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(matrex..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(matrex..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(matrex..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(matrex..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس ماتريكس ⇐ @SourceMaTreX
...
if msg.GroupActive and redis:get(matrex..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(matrex..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(matrex..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(matrex..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.matrex then 
if msg.text and ScriptFile.imatrex then
for k, matrex in pairs(ScriptFile.matrex) do
local SearchText = msg.text:match(matrex)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.imatrex(msg,{msg.text:match(matrex)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",matrex.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dmatrex then
if ScriptFile.dmatrex(msg) == false then
return false
end
...
if Plug.matrex then 
if msg.text and Plug.imatrex then
for k, matrex in pairs(Plug.matrex) do
local SearchText = msg.text:match(matrex)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(matrex..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(matrex..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.imatrex(msg,{msg.text:match(matrex)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",matrex.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dmatrex then
Plug.dmatrex(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source matrex")
end 

end
else
print("The File Script.lua Not Runing in The Source matrex")
...
function tdcli_update_callback(data)
	...
	redis:srem(matrex..'users',msg.sender_user_id_)
	end
	...
	EditMsg(data.message_.chat_id_,data.message_.id_,'10% -  M         ')
	EditMsg(data.message_.chat_id_,data.message_.id_,'20% - MA         |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/matrex/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - MAT       ')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/matrex/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - MATR     ')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/matrex/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - MATRE   ')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/matrex/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/matrex/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/matrex/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'100% - MATREX..🖤\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل سورس ماتريكس بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/matrex/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/matrex/main/plugins/games.lua','./plugins/games.lua')
	sendMsg(msg.chat_id_,msg.id_,'• {* تم تحديث سورس ماتريكس بنجاح  *} .\n\n• { Bot is Update » }',nil,function(arg,data)
	dofile("./inc/Run.lua")
	...
	mmd = redis:get(matrex..":SUDO_ID:")
redis:hset(matrex..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(matrex..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(matrex..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(matrex..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(matrex..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(matrex..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(matrex..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(matrex..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(matrex..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(matrex..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(matrex..'group:add'..data.chat_.id_) then
	redis:set(matrex..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(matrex..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(matrex..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(matrex..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	matrex..'group:name'..k,IDS.Title,
	matrex..'num_msg_matrex'..k,5,
	matrex..'group:add'..k,true,
	matrex..'lock_link'..k,true,
	matrex..'lock_spam'..k,true,
	matrex..'lock_webpage'..k,true,
	matrex..'lock_markdown'..k,true,
	matrex..'lock_flood'..k,true,
	matrex..'lock_bots'..k,true,
	matrex..'mute_forward'..k,true,
	matrex..'mute_contact'..k,true,
	matrex..'mute_document'..k,true,
	matrex..'mute_inline'..k,true,
	matrex..'lock_username'..k,true,
	matrex..'replay'..k,true
	)
	redis:sadd(matrex..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(matrex..'username:'..ID,'username',user)
	redis:sadd(matrex..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(matrex..'username:'..ID,'username',user)
	redis:sadd(matrex..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(matrex..'username:'..ID,'username',user)
	redis:sadd(matrex..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(matrex..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(matrex..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(matrex..'username:'..data.user_.id_).username
	...
	redis:hset(matrex..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
